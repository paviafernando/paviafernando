...
const text = {
  en: {
    ...
    run: "Run it locally",
    deploy: "Deploy",
    deployBody: "Every push runs the checks and publishes to Vercel from GitHub Actions. `main` goes to production, `develop` to staging, any other branch to a preview URL. Setup is in [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).",
  ...
  es: {
    ...
    deploy: "Deploy",
    deployBody: "Cada push corre los checks y publica en Vercel desde GitHub Actions. `main` va a producción, `develop` a staging y cualquier otra rama a una URL de preview. La configuración está en [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).",
  ...
  pt: {
    ...
    deploy: "Deploy",
    deployBody: "Cada push roda as verificações e publica na Vercel pelo GitHub Actions. `main` vai para produção, `develop` para staging e qualquer outra branch para uma URL de preview. A configuração está em [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).",
