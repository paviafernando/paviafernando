...
Current: Technical lead/senior .NET engineer at Innovate On Demand (Sep 2025-present, remote): a vehicle finance/lease platform (client product, name not public), Node.js dealer platforms (data ingestion and integrations including BigQuery, the Facebook Marketing API, Fortellis, and Gemini, used for natural-language-to-SQL generation to build marketing audiences), and, recently joined, a lead capture and booking platform for collision repair shops (client product, name not public). Founder/product lead of ComercIApp (2025-present, his own product): WhatsApp store/invoicing app for small Argentine shops with an AI sales agent, .NET 10 Web API + React + PostgreSQL, multi-tenant, offline-first, MercadoPago, Meta login/catalog sync (Instagram, Facebook, WhatsApp), two AI providers (Claude and Gemini, including AI-generated product images). ComercIApp Academy: tutorials site, Next.js/React frontend, headless WordPress (PHP/MySQL) backend.
...
Client work (finance/lease platform, translation portal, dealer platform, lease engine, collision-repair lead capture platform): NDA-like, never guess or invent names, say "client product, name not public".
